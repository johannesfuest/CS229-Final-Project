...
class GaussianMultinomialDiffusion(torch.nn.Module):
    def __init__(
        ...
        self.num_classes = num_classes.astype(np.int32) # it as a vector [K1, K2, ..., Km]
        self.num_classes_expanded = torch.from_numpy(
            np.concatenate([num_classes[i].repeat(num_classes[i]) for i in range(len(num_classes))])
        ).type(torch.IntTensor).to(device)
    ...
    def sample_from_known(self, x, y):
        ...
        z_norm = model_input[:, :self.num_numerical_features]
        log_z = model_input[:, self.num_numerical_features:]
